...
- [SYN Scanner](https://github.com/Nyveruus/systems-programming/tree/main/projects/networking/port-scanner) — TCP port scanner that never completes the handshake. It implements a detached thread for listening for SYN-ACKS and printing the open ports, while another thread is in charge of creating and dispatching raw SYN packets with a raw socket. IP/TCP headers and their checksums must and are manually computed.
