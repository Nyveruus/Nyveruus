...
- [SYN Scanner](https://github.com/Nyveruus/systems-programming/tree/main/projects/networking/port-scanner) — TCP port scanner that never completes the handshake. It implements a detached thread for listening for SYN-ACKS and printing the open ports, while another thread is in charge of creating and dispatching SYN packets with a raw socket. The raw socket and raw SYN packets are necessary for never comleting the 3-way TCP handshake, and IP/TCP headers along with their checksums must and are manually computed when building the packets.
...
- [Linux Audit Tool](https://github.com/Nyveruus/Linux-and-bash/tree/main/security/audit-tool) — Linux auditing and hardening tool for Debian. Uses check functions to output medium, low or critical warnings for vulnerabilities of different categories: accounts, SSH config, password policy and quality, file permissions. Prints summary at end. Implements argument parsing for arguments like selecting specific  categories to audit. Checks accounts: users with empty passwords in /etc/shadow, is root locked in /etc/shadow, duplicate UIDs, duplicate GUIDs, service accounts with interactive shells and users with sudoers; SSH config: PermitRootLogin, PasswordAuthentication, PermitEmptyPasswoeds, X11 Forwarding, default port, max # of SSH authentication attempts and ssh file key persmissions; password policy: password aging, password history, password quality with PAM; file permissions: file permisisons in important directories, world writable files without sticky bit in /tmp, SUID binaries except for common system SUIDs like passwd, sudo..., information on binaries with SGID, home directories have expected secure permissions.
